...
def obj_func_plotter(xs,means,stds,evals,turns):
    # Plot the the objective function at every turn in a colorplot
    ...
    p = ax.scatter(xs, turns, means, c=means, cmap='plasma',linewidth = 0, antialiased=False)
    ...
    s = ax.scatter(xs, turns, stds, c=stds, cmap='magma',linewidth = 0, antialiased=False)
...
# I need a function that will compute the synchronous phase 
# and detect when the rate of change is very large and make 
# individual ranges of dt for the optimization to cater to
# the 
...
# t_opt =  np.arange(0.01, t_arr[-1]+ dt_opt, dt_opt)
start_turn = 0
sim_arr = []
...
for i, entry in enumerate(sim_arr):
    ...
    if i == 0:
        ...
        # print('Phase at t = 0 ms is: ', phi_val.x)
    ...
    # Main loop goes here
        ...
        # logging.debug(f'{key} : {getsizeof(a[key])}')
        if i<=3:
        # phi_val = minimize_scalar(obj_func ,bounds = [0, np.pi*2], method = 'bounded', tol = 1e-1)
        ...
        # print('Phase at t = ', t_arr[i]*1000, ' ms is: ', phi_val.x)
        
        
        
        if i == 6:
            #get out of the loop and plot
            break
