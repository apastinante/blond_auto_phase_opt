...
class ProgramDef(object):
    def __init__(self,v_1,v_2,phase,time=None, sync_momentum = None, t_range = None, windowed = False, denominator_phase = 4):
        ...
        self.fig3.canvas.mpl_connect('close_event', self.close_plots)
...
class tolSetter(object):
    ...
    """
    def __init__(self, phase_diff, phase):
        ...
        self.ylab = 'Synchronous Phase Change [rad/turn]'
        self.ylab2 = 'Synchronous Phase [rad]'
        # Plot the date with the y axis ranging from 0 to 1.5 times the max

        ## Voltage h=1
        self.ax.set_title('Synchronous Phase Change for h=1 (tolerance set)')
        ...
        self.ax2.plot(self.phase, 'r',label = 'h=1' )
        self.ax2.set_title('Synchronous Phase for h=1 (points at which to sample)')

        self.ax2.set_xlabel(self.xlab)
        self.ax2.set_ylabel(self.ylab2)
    ...
    def onpick1(self,event):
        ...
        print('Tolerance set at: ', self.y)

        self.update_plot1()

    def update_plot1(self):
        self.ax.clear()
        self.ax.axhline(self.y,color = 'r', linestyle = '--')
    ...
    def onpick2(self,event):
        self.x2 , self.y2=  round(event.xdata), event.ydata
        
        # Check if any of the samples is within 1000 turns of the turn choice
        check = [abs(x - self.x2) < 1000 for x in self.samples]
        if any(check):
            print('Already sampled at turn: ', self.samples[check.index(True)])
        elif self.x2<=0:
            print('Invalid sampling at turn: ', self.x2)
        else:
            self.samples.append(self.x2)
            print('Sampling at turn: ', self.x2)
    ...
    def update_plot2(self):
        ...
        for sample in self.samples:
            self.ax2.axvline(sample, color = 'b', linestyle = '--')
...
class Waterfaller:
    def __init__(self, Profile, curr_turn, sampling_d_turns =100, n_turns = None, traces = 10000):
        """"
...
class ProgramDefOpt(object):
    def __init__(self,v_1,v_2,time=None, sync_momentum = None, t_range = None, windowed = False, denominator_phase = 4):
        ...
        # self.fig1.canvas.mpl_connect('close_event', self.close_plots1)
        # self.fig2.canvas.mpl_connect('close_event', self.close_plots2)
    ...
    def close_plots1(self,event):
        # When the figure is closed, close the remaining plots
        # Force them all to close

        plt.close('all')


    def close_plots2(self,event):
        # When the figure is closed, close the remaining plots
        # Force them all to close
        plt.close('all')
